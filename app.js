  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      if (collected.aircraft.length) {
        ...
        var ah = await supabase.from("aircraft_hours").insert(rows);
        if (ah.error) throw ah.error;
      }

      // 3) Subir archivos uno a uno
      var validFiles = selectedFiles.filter(function (s) { return !s.error; });
      for (var i = 0; i < validFiles.length; i++) {
        setFormStatus("Subiendo archivo " + (i+1) + " de " + validFiles.length + "…", "info");
        var fileEntry = validFiles[i];
        var file = fileEntry.file;
        var safeName = sanitizeFilename(file.name);
        var path = "feedbacks/" + feedbackId + "/" + Date.now() + "-" + i + "-" + safeName;
        ...
        var ff = await supabase.from("feedback_files").insert([{
